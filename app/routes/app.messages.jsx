...
function defaultGlobalSettings() {
  return {
    ...
    eta_preview_theme_font: "", // Deprecated: use preview_body_font instead
...
function defaultRule() {
  return {
    ...
    settings: {
      ...
      // Shipping Prices block
      show_shipping_prices: false,
      shipping_prices_divider: "|",
      shipping_prices_bg_color: "",
      shipping_prices_border_width: 0,
      shipping_prices_border_color: "#e5e7eb",
      shipping_prices_border_radius: 8,
      shipping_prices_text_color: "#374151",
      shipping_prices_alignment: "center",
      shipping_prices_icon_size: 16,
      // Option 1 (Standard)
      shipping_opt1_enabled: true,
      shipping_opt1_name: "Standard",
      shipping_opt1_price: "£3.99",
      shipping_opt1_time_auto: true,
      shipping_opt1_time_manual: "",
      shipping_opt1_icon: "truck",
      shipping_opt1_icon_style: "outline",
      // Option 2 (Express)
      shipping_opt2_enabled: false,
      shipping_opt2_name: "Express",
      shipping_opt2_price: "£6.99",
      shipping_opt2_time_auto: false,
      shipping_opt2_time_manual: "Next day",
      shipping_opt2_icon: "bolt",
      shipping_opt2_icon_style: "solid",
      // Option 3 (Free)
      shipping_opt3_enabled: true,
      shipping_opt3_mode: "static",
      shipping_opt3_icon: "gift",
      shipping_opt3_icon_style: "solid",
    },
  };
}
  ...
  useEffect(() => {
    if (rule?.id) {
      setCollapsedPanels({
        ...
        shipping_prices: getCollapsedState(rule.id, 'shipping_prices'),
      });
    }
  }, [rule?.id]);
              ...
              {rule && !showTypographyPanel && !showAlignmentPanel && !showGlobalSettingsPanel && (
                ...
                >
                  ...
                  <div
                    role="button"
                    tabIndex={0}
                    aria-expanded={!collapsedPanels.shipping_prices}
                    style={{
                      padding: "12px 16px",
                      display: "flex",
                      alignItems: "center",
                      justifyContent: "space-between",
                      background: !collapsedPanels.shipping_prices ? "var(--p-color-bg-surface-hover, #f8fafc)" : "var(--p-color-bg-surface, #ffffff)",
                      borderBottom: !collapsedPanels.shipping_prices ? "1px solid var(--p-color-border, #e5e7eb)" : "none",
                      cursor: "pointer",
                      outline: "none",
                    }}
                    onClick={() => togglePanel('shipping_prices')}
                    onKeyDown={(e) => {
                      if (e.key === "Enter" || e.key === " ") {
                        e.preventDefault();
                        togglePanel('shipping_prices');
                      }
                    }}
                  >
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      <span style={{ color: "var(--p-color-text-subdued, #6b7280)" }} aria-hidden="true">
                        {collapsedPanels.shipping_prices ? <ChevronRightIcon /> : <ChevronDownIcon />}
                      </span>
                      <s-heading>Shipping Prices</s-heading>
                    </div>
                    <label
                      style={{ display: "flex", alignItems: "center", gap: 6 }}
                      onClick={(e) => e.stopPropagation()}
                    >
                      <s-text size="small">{rule.settings?.show_shipping_prices ? "Enabled" : "Disabled"}</s-text>
                      <input
                        type="checkbox"
                        checked={!!rule.settings?.show_shipping_prices}
                        onChange={(e) => {
                          const next = [...rules];
                          next[safeSelectedIndex] = {
                            ...rule,
                            settings: {
                              ...rule.settings,
                              show_shipping_prices: e.target.checked,
                            },
                          };
                          setRules(next);
                          if (rule?.id) {
                            setCollapsedPanels(prev => ({
                              ...prev,
                              shipping_prices: !e.target.checked,
                            }));
                          }
                        }}
                      />
                  ...
                  <div style={{ padding: 16, display: "flex", flexDirection: "column", gap: 16 }}>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 8, padding: 12, border: "1px solid var(--p-color-border, #e5e7eb)", borderRadius: 8 }}>
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                        <s-text variant="headingSm">Option 1: Standard Delivery</s-text>
                        <label style={{ display: "flex", alignItems: "center", gap: 6 }}>
                          <s-text size="small">{rule.settings?.shipping_opt1_enabled !== false ? "Enabled" : "Disabled"}</s-text>
                          <input
                            type="checkbox"
                            checked={rule.settings?.shipping_opt1_enabled !== false}
                            onChange={(e) => {
                              const next = [...rules];
                              next[safeSelectedIndex] = { ...rule, settings: { ...rule.settings, shipping_opt1_enabled: e.target.checked } };
                              setRules(next);
                            }}
                          />
                        </label>
                      ...
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                        <label>
                          ...
                          <input
                            type="text"
                            value={rule.settings?.shipping_opt1_name || "Standard"}
                            onChange={(e) => {
                              const next = [...rules];
                              next[safeSelectedIndex] = { ...rule, settings: { ...rule.settings, shipping_opt1_name: e.target.value } };
                              setRules(next);
                            }}
                            style={{ width: "100%" }}
                          />
                        ...
                        <label>
                          <s-text size="small">Price</s-text>
                          <input
                            type="text"
                            value={rule.settings?.shipping_opt1_price || "£3.99"}
                            onChange={(e) => {
                              const next = [...rules];
                              next[safeSelectedIndex] = { ...rule, settings: { ...rule.settings, shipping_opt1_price: e.target.value } };
                              setRules(next);
                            }}
                            style={{ width: "100%" }}
                          />
                      ...
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                        <label style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          <input
                            type="checkbox"
                            checked={rule.settings?.shipping_opt1_time_auto !== false}
                            onChange={(e) => {
                              const next = [...rules];
                              next[safeSelectedIndex] = { ...rule, settings: { ...rule.settings, shipping_opt1_time_auto: e.target.checked } };
                              setRules(next);
                            }}
                          />
                          <s-text size="small">Auto time from ETA</s-text>
                        </label>
                        {rule.settings?.shipping_opt1_time_auto === false && (
                          <label>
                            <s-text size="small">Manual Time</s-text>
                            <input
                              type="text"
                              value={rule.settings?.shipping_opt1_time_manual || ""}
                              placeholder="e.g. 2-3 days"
                              onChange={(e) => {
                                const next = [...rules];
                                next[safeSelectedIndex] = { ...rule, settings: { ...rule.settings, shipping_opt1_time_manual: e.target.value } };
                                setRules(next);
                              }}
                              style={{ width: "100%" }}
                            />
                      ...
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                        <label>
                          <s-text size="small">Icon</s-text>
                          <select
                            value={rule.settings?.shipping_opt1_icon || "truck"}
                            onChange={(e) => {
                              const next = [...rules];
                              next[safeSelectedIndex] = { ...rule, settings: { ...rule.settings, shipping_opt1_icon: e.target.value } };
                              setRules(next);
                            }}
                            style={{ width: "100%" }}
                          >
                            <option value="">None</option>
                            <option value="truck">Truck</option>
                            <option value="cube">Box/Package</option>
                            <option value="clock">Clock</option>
                            <option value="calendar">Calendar</option>
                            <option value="map-pin">Location Pin</option>
                          </select>
                        </label>
                        <label>
                          <s-text size="small">Icon Style</s-text>
                          <select
                            value={rule.settings?.shipping_opt1_icon_style || "outline"}
                            onChange={(e) => {
                              const next = [...rules];
                              next[safeSelectedIndex] = { ...rule, settings: { ...rule.settings, shipping_opt1_icon_style: e.target.value } };
                              setRules(next);
                            }}
                            style={{ width: "100%" }}
                          >
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 8, padding: 12, border: "1px solid var(--p-color-border, #e5e7eb)", borderRadius: 8 }}>
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                        <s-text variant="headingSm">Option 2: Express Delivery</s-text>
                        <label style={{ display: "flex", alignItems: "center", gap: 6 }}>
                          <s-text size="small">{rule.settings?.shipping_opt2_enabled ? "Enabled" : "Disabled"}</s-text>
                          <input
                            type="checkbox"
                            checked={!!rule.settings?.shipping_opt2_enabled}
                            onChange={(e) => {
                              const next = [...rules];
                              next[safeSelectedIndex] = { ...rule, settings: { ...rule.settings, shipping_opt2_enabled: e.target.checked } };
                              setRules(next);
                            }}
                          />
                        </label>
                      ...
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                        <label>
                          ...
                          <input
                            type="text"
                            value={rule.settings?.shipping_opt2_name || "Express"}
                            onChange={(e) => {
                              const next = [...rules];
                              next[safeSelectedIndex] = { ...rule, settings: { ...rule.settings, shipping_opt2_name: e.target.value } };
                              setRules(next);
                            }}
                            style={{ width: "100%" }}
                          />
                        ...
                        <label>
                          ...
                          <input
                            type="text"
                            value={rule.settings?.shipping_opt2_price || "£6.99"}
                            onChange={(e) => {
                              const next = [...rules];
                              next[safeSelectedIndex] = { ...rule, settings: { ...rule.settings, shipping_opt2_price: e.target.value } };
                              setRules(next);
                            }}
                            style={{ width: "100%" }}
                          />
                      ...
                      <label>
                        <s-text size="small">Delivery Time</s-text>
                        <input
                          type="text"
                          value={rule.settings?.shipping_opt2_time_manual || "Next day"}
                          onChange={(e) => {
                            const next = [...rules];
                            next[safeSelectedIndex] = { ...rule, settings: { ...rule.settings, shipping_opt2_time_manual: e.target.value } };
                            setRules(next);
                          }}
                          style={{ width: "100%" }}
                        />
                      ...
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                        <label>
                          <s-text size="small">Icon</s-text>
                          <select
                            value={rule.settings?.shipping_opt2_icon || "bolt"}
                            onChange={(e) => {
                              const next = [...rules];
                              next[safeSelectedIndex] = { ...rule, settings: { ...rule.settings, shipping_opt2_icon: e.target.value } };
                              setRules(next);
                            }}
                            style={{ width: "100%" }}
                          >
                        ...
                        <label>
                          <s-text size="small">Icon Style</s-text>
                          <select
                            value={rule.settings?.shipping_opt2_icon_style || "solid"}
                            onChange={(e) => {
                              const next = [...rules];
                              next[safeSelectedIndex] = { ...rule, settings: { ...rule.settings, shipping_opt2_icon_style: e.target.value } };
                              setRules(next);
                            }}
                            style={{ width: "100%" }}
                          >
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 8, padding: 12, border: "1px solid var(--p-color-border, #e5e7eb)", borderRadius: 8 }}>
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                        <s-text variant="headingSm">Option 3: Free Delivery</s-text>
                        <label style={{ display: "flex", alignItems: "center", gap: 6 }}>
                          <s-text size="small">{rule.settings?.shipping_opt3_enabled !== false ? "Enabled" : "Disabled"}</s-text>
                          <input
                            type="checkbox"
                            checked={rule.settings?.shipping_opt3_enabled !== false}
                            onChange={(e) => {
                              const next = [...rules];
                              next[safeSelectedIndex] = { ...rule, settings: { ...rule.settings, shipping_opt3_enabled: e.target.checked } };
                              setRules(next);
                            }}
                          />
                        </label>
                      ...
                      <label>
                        <s-text size="small">Mode</s-text>
                        <select
                          value={rule.settings?.shipping_opt3_mode || "static"}
                          onChange={(e) => {
                            const next = [...rules];
                            next[safeSelectedIndex] = { ...rule, settings: { ...rule.settings, shipping_opt3_mode: e.target.value } };
                            setRules(next);
                          }}
                          style={{ width: "100%" }}
                        >
                          <option value="static">Static: "Free over £X"</option>
                          <option value="dynamic">Dynamic: "£X more for FREE"</option>
                        </select>
                      </label>
                      <s-text size="small" style={{ color: "var(--p-color-text-subdued)" }}>
                        {rule.settings?.shipping_opt3_mode === "dynamic"
                          ? "Shows remaining amount to qualify for free delivery (calculated from cart total)"
                          : "Shows the free delivery threshold from your Free Delivery settings"}
                      </s-text>
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                        <label>
                          <s-text size="small">Icon</s-text>
                          <select
                            value={rule.settings?.shipping_opt3_icon || "gift"}
                            onChange={(e) => {
                              const next = [...rules];
                              next[safeSelectedIndex] = { ...rule, settings: { ...rule.settings, shipping_opt3_icon: e.target.value } };
                              setRules(next);
                            }}
                            style={{ width: "100%" }}
                          >
                        ...
                        <label>
                          <s-text size="small">Icon Style</s-text>
                          <select
                            value={rule.settings?.shipping_opt3_icon_style || "solid"}
                            onChange={(e) => {
                              const next = [...rules];
                              next[safeSelectedIndex] = { ...rule, settings: { ...rule.settings, shipping_opt3_icon_style: e.target.value } };
                              setRules(next);
                            }}
                            style={{ width: "100%" }}
                          >
                  ...
                  >
                    {rule ? (
                      ...
                      >
                        ...
                        {/* Shipping Prices Preview - shown below special delivery when enabled */}
                        {rule.settings?.show_shipping_prices && (
                          <div style={{ width: "100%", maxWidth: "100%" }}>
                            {(() => {
                              const divider = rule.settings?.shipping_prices_divider || "|";
                              const alignment = rule.settings?.shipping_prices_alignment || "center";
                              const bgColor = rule.settings?.shipping_prices_bg_color || "";
                              const textColor = rule.settings?.shipping_prices_text_color || "#374151";
                              const borderWidth = rule.settings?.shipping_prices_border_width ?? 0;
                              const borderColor = rule.settings?.shipping_prices_border_color || "#e5e7eb";
                              const borderRadius = rule.settings?.shipping_prices_border_radius ?? 8;
                              const iconSize = rule.settings?.shipping_prices_icon_size ?? 16;

                              // Build options array
                              const options = [];

                              // Option 1: Standard
                              if (rule.settings?.shipping_opt1_enabled !== false) {
                                const name = rule.settings?.shipping_opt1_name || "Standard";
                                const price = rule.settings?.shipping_opt1_price || "£3.99";
                                const time = rule.settings?.shipping_opt1_time_auto !== false
                                  ? (rule.settings?.eta_delivery_days_min === rule.settings?.eta_delivery_days_max
                                    ? (rule.settings?.eta_delivery_days_min || 3) + "d"
                                    : (rule.settings?.eta_delivery_days_min || 3) + "-" + (rule.settings?.eta_delivery_days_max || 5) + "d")
                                  : (rule.settings?.shipping_opt1_time_manual || "");
                                const icon = rule.settings?.shipping_opt1_icon || "";
                                const iconStyle = rule.settings?.shipping_opt1_icon_style || "outline";
                                options.push({ name, price, time, icon, iconStyle });
                              }

                              // Option 2: Express
                              if (rule.settings?.shipping_opt2_enabled) {
                                const name = rule.settings?.shipping_opt2_name || "Express";
                                const price = rule.settings?.shipping_opt2_price || "£6.99";
                                const time = rule.settings?.shipping_opt2_time_manual || "Next day";
                                const icon = rule.settings?.shipping_opt2_icon || "";
                                const iconStyle = rule.settings?.shipping_opt2_icon_style || "solid";
                                options.push({ name, price, time, icon, iconStyle });
                              }

                              // Option 3: Free
                              if (rule.settings?.shipping_opt3_enabled !== false) {
                                const mode = rule.settings?.shipping_opt3_mode || "static";
                                const threshold = globalSettings?.fd_threshold || 3500;
                                const thresholdFormatted = (threshold / 100).toFixed(threshold % 100 === 0 ? 0 : 2);
                                const currencySymbol = globalSettings?.fd_currency === "USD" ? "$" : globalSettings?.fd_currency === "EUR" ? "€" : "£";
                                const text = mode === "dynamic"
                                  ? currencySymbol + "X more for FREE"
                                  : "Free over " + currencySymbol + thresholdFormatted;
                                const icon = rule.settings?.shipping_opt3_icon || "";
                                const iconStyle = rule.settings?.shipping_opt3_icon_style || "solid";
                                options.push({ name: text, price: "", time: "", icon, iconStyle, isFree: true });
                              }
                                ...
                                >
                                  {options.map((opt, idx) => (
                                    <div key={idx} style={{ display: "flex", alignItems: "center", gap: 4 }}>
                                      {opt.icon && (
                                        <span style={{ width: iconSize, height: iconSize, color: textColor, flexShrink: 0 }}>
                                          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill={opt.iconStyle === "solid" ? "currentColor" : "none"} stroke={opt.iconStyle === "outline" ? "currentColor" : "none"} strokeWidth={opt.iconStyle === "outline" ? 1.5 : 0} style={{ width: "100%", height: "100%" }}>
                                            {opt.icon === "truck" && <path strokeLinecap="round" strokeLinejoin="round" d="M8.25 18.75a1.5 1.5 0 0 1-3 0m3 0a1.5 1.5 0 0 0-3 0m3 0h6m-9 0H3.375a1.125 1.125 0 0 1-1.125-1.125V14.25m17.25 4.5a1.5 1.5 0 0 1-3 0m3 0a1.5 1.5 0 0 0-3 0m3 0h1.125c.621 0 1.129-.504 1.09-1.124a17.902 17.902 0 0 0-3.213-9.193 2.056 2.056 0 0 0-1.58-.86H14.25M16.5 18.75h-2.25m0-11.177v-.958c0-.568-.422-1.048-.987-1.106a48.554 48.554 0 0 0-10.026 0 1.106 1.106 0 0 0-.987 1.106v7.635m12-6.677v6.677m0 4.5v-4.5m0 0h-12" />}
                                            {opt.icon === "bolt" && <path fillRule="evenodd" d="M14.615 1.595a.75.75 0 0 1 .359.852L12.982 9.75h7.268a.75.75 0 0 1 .548 1.262l-10.5 11.25a.75.75 0 0 1-1.272-.71l1.992-7.302H3.75a.75.75 0 0 1-.548-1.262l10.5-11.25a.75.75 0 0 1 .913-.143Z" clipRule="evenodd" />}
                                            {opt.icon === "gift" && <path strokeLinecap="round" strokeLinejoin="round" d="M21 11.25v8.25a1.5 1.5 0 0 1-1.5 1.5H5.25a1.5 1.5 0 0 1-1.5-1.5v-8.25M12 4.875A2.625 2.625 0 1 0 9.375 7.5H12m0-2.625V7.5m0-2.625A2.625 2.625 0 1 1 14.625 7.5H12m0 0V21m-8.625-9.75h18c.621 0 1.125-.504 1.125-1.125v-1.5c0-.621-.504-1.125-1.125-1.125h-18c-.621 0-1.125.504-1.125 1.125v1.5c0 .621.504 1.125 1.125 1.125Z" />}
                                            {opt.icon === "cube" && <path strokeLinecap="round" strokeLinejoin="round" d="m21 7.5-9-5.25L3 7.5m18 0-9 5.25m9-5.25v9l-9 5.25M3 7.5l9 5.25M3 7.5v9l9 5.25m0-9v9" />}
                                            {opt.icon === "clock" && <path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />}
                                            {opt.icon === "rocket" && <path strokeLinecap="round" strokeLinejoin="round" d="M15.59 14.37a6 6 0 0 1-5.84 7.38v-4.8m5.84-2.58a14.98 14.98 0 0 0 6.16-12.12A14.98 14.98 0 0 0 9.631 8.41m5.96 5.96a14.926 14.926 0 0 1-5.841 2.58m-.119-8.54a6 6 0 0 0-7.381 5.84h4.8m2.581-5.84a14.927 14.927 0 0 0-2.58 5.84m2.699 2.7c-.103.021-.207.041-.311.06a15.09 15.09 0 0 1-2.448-2.448 14.9 14.9 0 0 1 .06-.312m-2.24 2.39a4.493 4.493 0 0 0-1.757 4.306 4.493 4.493 0 0 0 4.306-1.758M16.5 9a1.5 1.5 0 1 1-3 0 1.5 1.5 0 0 1 3 0Z" />}
                                            {opt.icon === "check-circle" && <path fillRule="evenodd" d="M2.25 12c0-5.385 4.365-9.75 9.75-9.75s9.75 4.365 9.75 9.75-4.365 9.75-9.75 9.75S2.25 17.385 2.25 12Zm13.36-1.814a.75.75 0 1 0-1.22-.872l-3.236 4.53L9.53 12.22a.75.75 0 0 0-1.06 1.06l2.25 2.25a.75.75 0 0 0 1.14-.094l3.75-5.25Z" clipRule="evenodd" />}
                                            {opt.icon === "star" && <path fillRule="evenodd" d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.006 5.404.434c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.434 2.082-5.005Z" clipRule="evenodd" />}
                                            {opt.icon === "heart" && <path fillRule="evenodd" d="M3.172 5.172a4 4 0 0 1 5.656 0L10 6.343l1.172-1.171a4 4 0 1 1 5.656 5.656L10 17.657l-6.828-6.829a4 4 0 0 1 0-5.656z" clipRule="evenodd" />}
                                            {opt.icon === "sparkles" && <path fillRule="evenodd" d="M9 4.5a.75.75 0 0 1 .721.544l.813 2.846a3.75 3.75 0 0 0 2.576 2.576l2.846.813a.75.75 0 0 1 0 1.442l-2.846.813a3.75 3.75 0 0 0-2.576 2.576l-.813 2.846a.75.75 0 0 1-1.442 0l-.813-2.846a3.75 3.75 0 0 0-2.576-2.576l-2.846-.813a.75.75 0 0 1 0-1.442l2.846-.813A3.75 3.75 0 0 0 7.466 7.89l.813-2.846A.75.75 0 0 1 9 4.5ZM18 1.5a.75.75 0 0 1 .728.568l.258 1.036c.236.94.97 1.674 1.91 1.91l1.036.258a.75.75 0 0 1 0 1.456l-1.036.258c-.94.236-1.674.97-1.91 1.91l-.258 1.036a.75.75 0 0 1-1.456 0l-.258-1.036a2.625 2.625 0 0 0-1.91-1.91l-1.036-.258a.75.75 0 0 1 0-1.456l1.036-.258a2.625 2.625 0 0 0 1.91-1.91l.258-1.036A.75.75 0 0 1 18 1.5ZM16.5 15a.75.75 0 0 1 .712.513l.394 1.183c.15.447.5.799.948.948l1.183.395a.75.75 0 0 1 0 1.422l-1.183.395c-.447.15-.799.5-.948.948l-.395 1.183a.75.75 0 0 1-1.422 0l-.395-1.183a1.5 1.5 0 0 0-.948-.948l-1.183-.395a.75.75 0 0 1 0-1.422l1.183-.395c.447-.15.799-.5.948-.948l.395-1.183A.75.75 0 0 1 16.5 15Z" clipRule="evenodd" />}
                                          </svg>
                                        </span>
                                      )}
                                      <span style={{ fontWeight: 500 }}>{opt.name}</span>
                                      {opt.price && <span>{opt.price}</span>}
                                      {opt.time && <span style={{ opacity: 0.7 }}>• {opt.time}</span>}
                                      {idx < options.length - 1 && (
                                        <span style={{ marginLeft: 8, opacity: 0.4 }}>{divider}</span>
                                      )}
                                    </div>
                                  ))}
                                </div>
