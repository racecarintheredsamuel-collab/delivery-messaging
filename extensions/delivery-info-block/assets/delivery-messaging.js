...
(function(){
  ...
  function updatePricingLevels(state) {
    ...
    document.querySelectorAll('.dib-pricing[data-levels]').forEach(el => {
      ...
      if (!el.dataset.jsInitialized) {
        ...
        setTimeout(() => {
          try {
            ...
            // Get fresh state when delay completes
            const currentState = dm.getState();
            const currentCartTotal = currentState.cartTotal || 0;
            const currentExcluded = currentState.excluded || false;
            freshEl.dataset.lastCartTotal = String(currentCartTotal);
            freshEl.dataset.lastExcluded = String(currentExcluded);
            renderPricing(freshEl, currentCartTotal, currentExcluded, fdThreshold, dm);
          } catch (err) {
      ...
      // After delay: only update if cart total OR excluded state has changed
      const lastCartTotal = parseInt(el.dataset.lastCartTotal || '-1', 10);
      const lastExcluded = el.dataset.lastExcluded === 'true';
      if (lastCartTotal === cartTotal && lastExcluded === excluded) {
        return;
      }
      el.dataset.lastCartTotal = String(cartTotal);
      el.dataset.lastExcluded = String(excluded);
      renderPricing(el, cartTotal, excluded, fdThreshold, dm);
    });
  }

  function renderPricing(el, cartTotal, excluded, fdThreshold, dm) {
      const excludedBehavior = el.dataset.excludedCartBehavior || 'message';
      const excludedMessage = el.dataset.excludedCartMessage || 'Delivery calculated at checkout';

      // Get multiMatch and check if this product is excluded
      const multiMatch = dm.getState().multiMatch || false;
      const isExcludedProduct = el.dataset.isExcludedProduct === 'true';

      // Apply exclusion behavior only if:
      // - multiMatch (2+ excluded items) → apply to ALL products
      // - OR single excluded item AND current product is NOT excluded
      const shouldApplyExclusion = excluded && (multiMatch || !isExcludedProduct);

      if (shouldApplyExclusion) {
        if (excludedBehavior === 'hide') {
          // Hide the entire line (includes icon), not just the pricing text
          const line = el.closest('.dib-line');
          if (line) {
        ...
        // Show the excluded message
        el.style.display = '';
        const subdued = el.dataset.excludedCartSubdued === 'true';
        const targetOpacity = subdued ? '0.7' : '1';
        // Apply subdued styling via class and inline styles for maximum compatibility
        el.classList.toggle('dib-subdued', subdued);
        el.style.fontStyle = subdued ? 'italic' : '';
        const newHtml = parseBold(excludedMessage);
      ...
      // Not excluded - ensure entire line is visible (styling reset happens after fade)
      el.style.display = '';
      const line = el.closest('.dib-line');
      if (line) {
        line.style.display = '';
      ...
      if (thresholdMessage && fdThreshold > 0 && cartTotal >= fdThreshold) {
        ...
        if (el.innerHTML !== newHtml) {
          ...
          setTimeout(() => {
            el.innerHTML = newHtml;
            el.style.fontStyle = '';
            el.classList.remove('dib-subdued');
            el.style.opacity = '1';
          }, 400);
        } else {
          el.style.fontStyle = '';
      ...
      // Find the appropriate level based on cart total
      // Levels are ordered: level 0 has threshold for "cart < X", level 1 for "X <= cart < Y", etc.
      // The last level (with threshold null) is "cart >= previous threshold"
      ...
      for (let i = 0; i < levels.length; i++) {
        ...
        if (threshold === null || threshold === undefined) {
          // No upper limit - this level applies for cart >= previous level's threshold
          selectedLevel = level;
          break;
        }

        if (cartTotal < threshold) {
          // Cart is below this level's threshold
          selectedLevel = level;
          break;
        }

        // Cart is >= this threshold, continue to next level
        if (i === levels.length - 1) {
          // Last level with a threshold - use it anyway
          selectedLevel = level;
        }
      }

      // Render the selected level
      const segments = selectedLevel.segments || [];
      const divider = selectedLevel.divider || '|';
      const daysDivider = selectedLevel.days_divider || '•';
      const showDays = selectedLevel.show_days !== false;
      const freeTextEnabled = selectedLevel.free_text_enabled;
      const freeText = freeTextEnabled ? (selectedLevel.free_text || '') : '';

      const parts = [];
      for (const segment of segments) {
        if (!segment.label && !segment.cost) continue;
        let part = segment.label || '';
        if (segment.cost != null && segment.cost > 0) {
          const costStr = dm.formatMoney(segment.cost);
      ...
      if (freeText) {
        const thresholdFormatted = dm.formatMoney(fdThreshold);
        parts.push(freeText.replace(/\{threshold\}/g, thresholdFormatted));
      }
  ...
  function processInitialBold() {
    document.querySelectorAll('.dib-pricing').forEach(el => {
      if (el.textContent.includes('**')) {
        el.innerHTML = parseBold(el.textContent);
      }
    });
  }
  ...
  function init() {
    // Process bold formatting in initial Liquid-rendered content
    processInitialBold();

    if (window.DeliveryMessaging && window.DeliveryMessaging.subscribe) {
      window.DeliveryMessaging.subscribe(updatePricingLevels);
    } else {
      // Retry after a short delay if DeliveryMessaging isn't ready
      setTimeout(init, 100);
    }
  }
